...
async function testHydrationEndpoints() {
  ...
  try {
    ...
    const summaryResponse = await axios.get(`${BASE_URL}/hydration/summary?date=${today}`);
    ...
    const entriesResponse = await axios.get(`${BASE_URL}/hydration?date=${today}`);
    ...
    const updatedSummaryResponse = await axios.get(`${BASE_URL}/hydration/summary?date=${today}`);
    ...
    console.log(`- Total after creation: ${summaryResponse.data.totalLiters} L`);
    console.log(`- Deleted 1 entry (500ml)`);
    console.log(`- Total after deletion: ${updatedSummaryResponse.data.totalLiters} L`);
...
async function testValidation() {
  ...
  try {
    ...
    try {
      ...
      console.log('✅ Validation works for negative amountMl:', error.response.data.error.message);
    }
    ...
    try {
      ...
      console.log('✅ Validation works for missing takenAt:', error.response.data.error.message);
    }
    ...
    try {
      ...
      console.log('✅ Validation works for invalid date:', error.response.data.error.message);
    }
